...
| violence | block | Default to blocking. **Correction, confirmed with real data:** the original assumption below was only checked against "defeat"-style phrasing and was never verified against "kill." It does not hold for "kill." "How do I kill the final boss in [game]" reliably scores `violence` as flagged (~0.52); swapping only the verb to "defeat" (~0.13) or "beat" (~0.18) does not trigger it — same sentence, same game, only the verb differs. This is a real, reproducible false positive on ordinary gaming questions, not a hypothetical. Deliberately not patched: any carve-out for "violence is fine when it's about a game" is structurally identical to the fictional-framing jailbreak technique (see `jailbreak_seeds.py`'s `hypothetical_fiction_framing` archetype) — loosening this risks a bypass path for real harmful requests wrapped in game framing. Over-blocking is the accepted trade-off for now; original note below kept for context. | Sadaf, correction: Zaid |
| ~~violence (original note, superseded above)~~ | ~~block~~ | ~~Gaming/fictional violence ("how do I defeat this boss") rarely trips this category in practice — the Moderation API is tuned for genuine violent content, not fictional framing, so we're not carving out a special case without evidence it's needed. Revisit if the test set shows false positives on fictional/gaming inputs.~~ | Sadaf |
